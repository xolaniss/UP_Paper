...
in order to meet said requirements.
\textcolor{red}{However, this can vary depending on the banking system. For example, a healthier banking system can sustain higher lending capacity or regulatory pressure can prompt more risk taking}
\citep[see][]{merrino2024macroprudential}. Therefore inclusion and
...
inclusion to determine if these initiatives meet their intended
goals\footnote{\textcolor{red}{We make the distinction between macroprudential and financial inclusion regulation for the purposes of the analysis. However, the focus of the paper is broader in nature and is primarily about bank regulation. The specific regulations that are included in the narrative indices can be found in Appendix A.6}}.
...
objectives.\footnote{\textcolor{red}{We note that these results are specific to bank lending to all borrowers. Non-bank financial institutions (NBFIs) also provide lending. Our analysis does not capture the impacts of inclusion focused regulatory developments on the lending by these NBFIs.}}
...
regulations. We consult circulars issued by the SARB to commercial banks
in South Africa, annual reports of commercial banks and SARB and risk
and capital management reports of commercial banks. We consider reports
from only the big 4 commercial banks in South Africa, as they account
for over 90 percent of banking industry
...
\begin{equation}\phantomsection\label{eq-weight}{
...
lending volumes. Our analysis covers
\textcolor{red}{January 2009 to February 2020. We estimate using robust standard errors clustered at the bank level}
\citep[see][]{zeileis2004}.
\textcolor{red}{This ensures estimates that are robust to heteroskedasticity and serial correlation. In addition, the clustering ensures that the model understands that the banks are from the same population}
\citep[see][]{fang2022bank}.

The three month change in lending is calculated as the log difference in
lending at \(t\) and \(t-3\).\footnote{We perform a visual test of the
  correlations between the response variables and our narrative indices.
  ...
  and A.8. We find a stronger co-movement between the narrative
  ...
  changes.}
\textcolor{red}{The log differences are then multiplied by 100.} This
approach is consistent with \cite{aiyar2016does}, \cite{deli2017real},
\cite{fang2022bank} and \cite{mirzaei2022effectiveness}.

\begin{equation}\phantomsection\label{eq-rate}{
Rate_{b,t}^{c} = \alpha_1Index_t^i + \tau_b^c + \lambda_t^c + \alpha'\Omega^c + \epsilon_{b,t}^c
}\end{equation}

\begin{equation}\phantomsection\label{eq-lending}{
...
the bank level lending rate. \(\Omega^c\) is a matrix of controls
\textcolor{red}{that include return on assets and total capital adequecy ratios measured at bank level, as well as the repo rate}.
\(\tau_b^c\) captures the bank fixed effect, \(\lambda_t^c\) are the
monthly time fixed effects, and \(\epsilon_{b,t}^c\) are the error
terms.
...
Tables 2 and 4 report the results from the estimation of
Equation~\ref{eq-rate} with standard errors clustered at bank level.
Tables 3 and 5 relate to Equation~\ref{eq-lending}, where the dependent
variable is the log difference in lending volumes between \(t\) and
\(t-3\), also clustered at bank
