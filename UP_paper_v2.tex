...
lending at \(t\) and \(t-3\).\footnote{We perform a visual test of the
  ...
  changes.}
\textcolor{red}{The log differences are then multiplied by a 100.} This
...
the bank level lending rate. \(\Omega^c\) is a matrix of controls, bank
level variables, and other macroeconomic variables. \(\tau_b^c\)
captures the bank fixed effect, \(\lambda_t^c\) are the monthly time
fixed effects, and \(\epsilon_{b,t}^c\) are the error terms.
